...
class Inventory(models.Model):
    product = models.OneToOneField(
        "core.Product", on_delete=models.CASCADE, related_name="inventory"
    )
    ...
    def __str__(self):
        return (
            f"Inventory for {self.product.name} - Current stock: {self.current_stock}"
        )
    ...
    def update_stock(self, adjustment, reason=""):
        ...
        InventoryAdjustment.objects.create(
            inventory=self, adjustment=adjustment, reason=reason
        )
...
class Purchase(models.Model):
    product = models.ForeignKey(
        "core.Product", on_delete=models.CASCADE, related_name="purchases"
    )
    ...
    inventory = models.ForeignKey(
        Inventory, on_delete=models.CASCADE, related_name="purchases"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Transaction(models.Model):
    product = models.ForeignKey(
        "core.Product", on_delete=models.CASCADE, related_name="transactions"
    )
    ...
    items = models.ManyToManyField(
        "core.Product", through="pos.Sale", related_name="transaction_items"
    )
    ...
    def get_top_customer(transactions):
        top_customer = (
            transactions.values("customer__name")
...
class SpecialDiscount(models.Model):
    product = models.ForeignKey(
        "core.Product", on_delete=models.CASCADE, related_name="special_discounts"
    )
    name = models.CharField(max_length=255)
    amount = models.DecimalField(
...
class StockAlert(models.Model):
    product = models.ForeignKey(
        "core.Product", on_delete=models.CASCADE, related_name="stock_alerts"
    )
...
class SalesAnalytics(models.Model):
    ...
    def generate_sales_analytics(cls, product_id, start_date, end_date):
        sales = Sale.objects.filter(
            product_id=product_id, transaction__date__range=(start_date, end_date)
        )
        total_sales_value = (
            sales.aggregate(total_value=Sum("subtotal"))["total_value"] or 0
        )
        top_selling_product = apps.get_model("core", "Product").objects.get(
            id=product_id
        )
...
class CustomerAnalytics(models.Model):
    ...
    def generate_customer_analytics(cls, customer_id):
        ...
        total_loyalty_points_earned = (
            transactions.aggregate(points=Sum("points_earned"))["points"] or 0
        )
        return cls.objects.create(
            customer=customer,
...
class Sale(models.Model):
    product = models.ForeignKey(
        "core.Product", on_delete=models.CASCADE, related_name="sales"
    )
    ...
    discount = models.ForeignKey(
        Discount, on_delete=models.CASCADE, null=True, blank=True, related_name="sales"
    )
...
class Return(models.Model):
    ...
    transaction = models.ForeignKey(
        Transaction, on_delete=models.CASCADE, related_name="returns"
    )
...
class Refund(models.Model):
    return_sale = models.ForeignKey(
        Return, on_delete=models.CASCADE, related_name="refunds", blank=True, null=True
    )
    transaction = models.ForeignKey(
